...
const float maxRateChange = 25.0; // Adjust this value as needed
float previousVel = 0;
...
void loop()
{
    ...
    */
    ...
    if (mpu.dmpGetCurrentFIFOPacket(FIFOBuffer))
    {
        ...
        // Apply rate control to 'vel'
        if ((vel >= 0 && previousVel >= 0) || (vel < 0 && previousVel < 0))
        {
            float rateChange = vel - previousVel;
            if (abs(rateChange) > maxRateChange)
            {
                if (rateChange > 0)
                {
                    vel = previousVel + maxRateChange;
                }
                else
                {
                    vel = previousVel - maxRateChange;
