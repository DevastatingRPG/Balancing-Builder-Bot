...
#define PID_MIN_LIMIT -200          // Min limit for PID
#define PID_MAX_LIMIT 200           // Max limit for PID
...
PIDParams pitchAgg(40, 185, 0.6);
PIDParams posCon(0, 0, 0);
PIDParams posAgg(0.001, 0, 0);
...
void loop()
{
    ...
    if (abs(Error_Pos) > 10)
    {
        ...
        stopping = true;
        // pitchPID.SetTunings(pitchCon._kp, )
    }
    else
    {
        Setpoint_Pitch = 0;
...
void rotateMotor(int speed1, int speed2)
{
    ...
    speed1 = constrain(speed1, MIN_ABSOLUTE_SPEED, 200);
    speed2 = constrain(speed2, MIN_ABSOLUTE_SPEED, 200);
