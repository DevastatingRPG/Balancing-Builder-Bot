...
#define SETPOINT_PITCH_ANGLE_OFFSET 0.666 // SETPOINT_PITCH_ANGLE_OFFSET: Setpoint pitch angle offset
...
PIDParams pitchTurn(28, 250, 2.5);
...
void loop()
{
    ...
    if ((turning == 1 && abs(Input_Pitch) > 7) || (turning == -1 && abs(Input_Pitch) > 6))
    {
        turning = 0;
        // Setpoint_Pitch = -0.5;
        Setpoint_Pos = -20;
    ...
    if (turning)
    {
        if ((millis() - turnStart) < 16)
        {
            rotateMotor(turning * abs(vel), (-turning) * abs(vel));
        }
        else
        {
            turning = 0;
