...
// PIDParams pitchCon(29, 300, 2.3);      // pitchCon: PID parameters for pitch control
// PIDParams posAgg(0.015, 0.01, 0.003); // posAgg: PID parameters for position control
...
//     vel += 0.7;
//   }
//   if (turning != 0 && abs(vel)<80){
//     vel = 100;
//   }
//   if (turning == 1)
//   {
//     rotateMotor(abs(vel), -abs(vel));
//     delay(80);
//     Setpoint_Pos = 0;
//     wheel_pulse_count_left = 0;
//     wheel_pulse_count_right = 0;
...
// PID parameter instances
PIDParams pitchCon(32, 300, 2.3); // pitchCon: PID parameters for pitch control
PIDParams pitchTurn(25, 300, 3.0);
PIDParams posAgg(0.015, 0.01, 0.003); // posAgg: PID parameters for position control
PIDParams yaw(0.5, 0.5, 0);           // yaw: PID parameters for yaw control
...
void loop()
{
    ...
    if (bluetooth.available() > 0)
    {
        ...
        else if (btData == 2)
        {
            ...
            wheel_pulse_count_right = 0;
            tone(12, 1000, 100);
            pitchPID.SetTunings(pitchTurn._kp, pitchTurn._kd, pitchTurn._ki);
            turning = 1; // Rotate left
            turnStart = millis();
            bluetooth.flush();
        }
    ...
    if (turning == 1 && (millis() - turnStart) < 80)
    {
        rotateMotor(abs(vel), -abs(vel));
    }
    else if (turning == 2 && (millis() - turnStart) < 80)
    {
        rotateMotor(-abs(vel), abs(vel));
    }
    else if (turning == 1 || turning == 2)
    {
        turning = 0;
        moving = false;
        wheel_pulse_count_left = 0;
