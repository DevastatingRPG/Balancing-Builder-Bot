...
PIDParams pitchTurn(32, 200, 3.0);
...
PIDParams pitchCon(32, 300, 2); // pitchCon: PID parameters for pitch control
...
void loop()
{
    ...
    if ((turning == 1 && abs(Input_Pitch) > 7) || (turning == -1 && abs(Input_Pitch) > 5))
    {
        turning = 0;
        Setpoint_Pitch = 0;
        Setpoint_Pos = 0;
