...
def sysCall_init():
    ...
    self.target_pos = sim.getObjectPosition(self.body, -1)[0:2]
    self.pos_error = 0
...
def sysCall_sensing():
    ...
    # Calculate position error as a signed value considering both x and y coordinates
    current_pos = sim.getObjectPosition(self.body, -1)[0:2]
    delta_x = self.target_pos[0] - current_pos[0]
    delta_y = self.target_pos[1] - current_pos[1]
    self.pos_error = math.sqrt(delta_x**2 + delta_y**2)

    # Determine the sign of the position error based on the direction of movement
    if delta_x < 0 or delta_y < 0:
        self.pos_error = -self.pos_error

    message, data, data2 = sim.getSimulatorMessage()
    if message == sim.message_keypress:
        if data[0] == 2007:  # forward up arrow
            self.stationary = False
            self.setpoint_pitch = 0.01
            ...
            self.manual_rotate = 0
            self.pitch_error = self.setpoint_pitch - self.pitch
            # Update target position to the current position
            self.target_pos = sim.getObjectPosition(self.body, -1)[0:2]
            self.pos_error = 0
    else:
        self.manual_rotate = 0
