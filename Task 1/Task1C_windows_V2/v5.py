...
def sysCall_init():
    ...
    self.pid_pitch = PIDController(
        kp=4,  # Start with a reasonable value
        ki=0.8,  # Start with a reasonable value
        kd=5,  # Start with a reasonable value
        dt=0.5,
        integral_limit=5,
    ...
    self.pid_pos = PIDController(
        ...
        ki=5,  # Start with a reasonable value
...
#     if -45 <= deg <= 45:
#         pos = sim.getObjectPosition(self.body, -1)[1]
#         self.pos_error = self.setpoint_pos - pos
#     elif -135 < deg < -45:
...
#             if -45 <= deg <= 45:
#                 self.setpoint_pos = sim.getObjectPosition(self.body, -1)[1]
#             elif -135 < deg < -45:
#                 self.setpoint_pos = sim.getObjectPosition(self.body, -1)[0]
#             elif -180 < deg <= -135 or 135 < deg <= 180:
#                 self.setpoint_pos = sim.getObjectPosition(self.body, -1)[1]
#             elif 45 < deg < 135:
#                 self.setpoint_pos = sim.getObjectPosition(self.body, -1)[0]
...
def calculate_euclidean_distance(body_pos, cone_pos, setpoint_pos):
    """
    Calculate the Euclidean distance between body and setpoint positions.
    ...
    Returns:
    float: The Euclidean distance from body to setpoint.
    """
    delta_x = setpoint_pos[0] - body_pos[0]
    delta_y = setpoint_pos[1] - body_pos[1]
    distance = np.sqrt(delta_x**2 + delta_y**2)

    # Vector from body to setpoint
    vec_body_to_setpoint = np.array([delta_x, delta_y])
    
    # Vector from body to cone
    vec_body_to_cone = np.array([cone_pos[0] - body_pos[0], cone_pos[1] - body_pos[1]])
...
def sysCall_sensing():
    ...
    setpoint_pos = [self.setpoint_pos, body_pos[1]]  # Assuming setpoint_pos is along the y-axis

    self.pos_error = calculate_euclidean_distance(body_pos, cone_pos, setpoint_pos)
    ...
    if message == sim.message_keypress:
        if data[0] == 2007:  # forward up arrow
            ...
            # Reset position error
            self.setpoint_pos = sim.getObjectPosition(self.body, -1)[1]  # Assuming setpoint_pos is along the y-axis

            self.pos_error = 0
            self.pitch_error = self.setpoint_pitch - pitch

    # Debugging information
    print(f"Setpoint Pitch: {self.setpoint_pitch}, Setpoint Position: {self.setpoint_pos}")
    print(f"Position Error: {self.pos_error}, Pitch Error: {self.pitch_error}")
