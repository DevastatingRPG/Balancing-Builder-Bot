...
float originalPos = 0;
const unsigned long RESET_INTERVAL = 2000; // 700 milliseconds
const unsigned long RESET_DURATION = 300;  // 500 milliseconds
...
void loop()
{
  ...
  {
    if (abs(Error_Pos - Setpoint_Pos) > 20)
    {
      Input_Pos = Error_Pos;
      posPID.Compute(true);
  ...
  {
    unsigned long currentTime = millis();
    if (!inResetPhase && abs(Error_Pos - originalPos) < 50)
    {
      Setpoint_Pos = 0;
      wheel_pulse_count_left = 0;
      wheel_pulse_count_right = 0;
