...
double Setpoint_WheelDiff, Input_WheelDiff, Output_WheelDiff;  // PID variables for angle control
...
PIDParams pitchCon(58, 500, 3.5);  // pitchCon: PID parameters for pitch control
...
PIDParams maintain(8, 100, 0.01); // Maintaining angle
...
PID maintainPID(&Input_WheelDiff, &Output_WheelDiff, &Setpoint_WheelDiff, maintain._kp, maintain._ki, maintain._kd, DIRECT);
...
void setupPID()
{
    ...
    Setpoint_WheelDiff = 0;
    maintainPID.SetOutputLimits(-100, 100);
    maintainPID.SetMode(AUTOMATIC);
    maintainPID.SetSampleTime(PID_SAMPLE_TIME_IN_MILLI);
}
...
bool inResetPhase = false;
float originalPitch = 0;
const unsigned long RESET_INTERVAL = 700; // 2 seconds
const unsigned long RESET_DURATION = 500; // 1 second
...
void loop()
{
    ...
    if (bluetooth.available() > 0)
    {
        ...
        case 16: // Gradually bring Setpoint_Pitch back to 0
            tone(12, 1000, 100);
            gradualStop = true;
    ...
    if (gradualStop)
    {
        ...
        else if (Setpoint_Pitch < 0)
        {
            Setpoint_Pitch += 0.5;
        }
        if (abs(Setpoint_Pitch) < 0.5)
        {
            Setpoint_Pitch = 0;
            gradualStop = false;
            moving = false;
    ...
    if (moving)
    {
        ...
        if (!inResetPhase && (currentTime - lastResetTime >= RESET_INTERVAL))
        {
            Setpoint_Pitch = -2 * originalPitch;
            inResetPhase = true;
            lastResetTime = currentTime;
            tone(12, 1000, 100);
        }
        else if (inResetPhase && (currentTime - lastResetTime >= RESET_DURATION))
        {
            Setpoint_Pitch = originalPitch;
            inResetPhase = false;
            lastResetTime = currentTime;
            tone(12, 1000, 100);
        }
    ...
    {
        Input_WheelDiff = (wheel_pulse_count_left - wheel_pulse_count_right);
        if (abs(Input_WheelDiff) > 50)
        {
            maintainPID.Compute(true);
            velL = vel + Output_WheelDiff;
            velR = vel - Output_WheelDiff;
        }
        else
        {
            velL = vel;
